...
export class TodosRouter extends Router {
  ...
  private async controllerGet(_: Request, response: Response) {
    try {
      const result = await this.connection.get("todo").all();
  ...
  private async controllerPost(request: Request, response: Response) {
    try {
      ...
      const result = await this.connection.get("todo").create(title, description, status, userId);
      response.status(200).json(`Succes created Todo with id ${result.id}`);
    } catch (error) {
      response.status(400).json((error as Error).message);
    }
  }
